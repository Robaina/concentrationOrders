...
\begin{document}
  ...
  \begin{align}
    \begin{aligned}
      \label{eq:7}
      &z = \min_{\substack{ \log{x} \in \rm I\!R^n, \\ v \in \rm I\!R^n_{\geq 0}, \\ y^+, y^- \in \{0, 1\} }} \; \log{x_p} - \log{x_q}
      \\
      &\mathrm{s.t.}
      \\
      &Sv = 0
      ...
      \\
      &\Delta^{\circ} G_{r(j)} + RT \sum_i n_{ij} \log{x_i} - y_j^{(+, -)}M< 0 \; \forall j \in \mathrm{R_{\Delta^{\circ} G_{r}}}
      \\
      &\Delta^{\circ} G_{r}^{\dagger} - \epsilon \leq \Delta^{\circ} G_{r} \leq \Delta^{\circ} G_{r}^{\dagger} + \epsilon
      \\
      &\log{x}_{min} \leq \log{x} \leq \log{x}_{max}
